 ...
 * \enddot
 *
 * \section estructura_matematica Estructura Matemática del Filtro
 *
 * La fórmula de Lagrange implementada es:
 * \f[
 * h_m(2n-1) = \frac{(-1)^{n+m-1}}{(m-n)!(m-1+n)!(2n-1)} \cdot \prod_{k=1}^{2m}(m-k+\frac{1}{2})
 * \f]
 *
 * \dot
 * digraph lagrange_formula {
 *   rankdir=LR;
 *   node [shape=box, style=filled];
 *
 *   INPUT [label="Parámetro m", fillcolor=lightyellow];
 *   SIGN [label="Signo\n(-1)^(n+m-1)", fillcolor=lightpink];
 *   PRODUCT [label="Productorio\nΠ(m-k+0.5)", fillcolor=lightblue];
 *   FACTORIAL1 [label="Factorial\n(m-n)!", fillcolor=lightgreen];
 *   FACTORIAL2 [label="Factorial\n(m-1+n)!", fillcolor=lightgreen];
 *   INDEX [label="Índice\n(2n-1)", fillcolor=lightcyan];
 *   RESULT [label="Coeficiente\nh_m(2n-1)", fillcolor=lightgray];
 *
 *   INPUT -> SIGN;
 *   INPUT -> PRODUCT;
 *   INPUT -> FACTORIAL1;
 ...
 * \enddot
 *
 * La funciÃ³n implementa el algoritmo:
